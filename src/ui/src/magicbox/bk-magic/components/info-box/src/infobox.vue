                    ...
                    <div class="bk-dialog-style">
                        ...
                        <div class="bk-dialog-body" :class="[{'bk-dialog-default-status': type !== 'default'}, 'bk-dialog-' + type, type === 'default' && content === false ? 'p0' : '']">
                            ...
                            <div class="bk-dialog-row" v-if="type !== 'default'">
                                <img src="" alt="loading" class="bk-dialog-mark bk-dialog-loading" v-if="type === 'loading'">
                                <p v-else>
...
<script>
     ...
     *  @param hasHeader {Boolean} - 消息框是否显示头部
    ...
    export default {
        ...
        data () {
            return {
                ...
                closeIcon: true,
                theme: 'primary',
                confirm: this.t('infoBox.confirm'),
        ...
        computed: {
            ...
            calcStatusOpts () {
                ...
                switch (this.type) {
                    case 'loading':
                        opts.title = 'loading'
                        opts.subtitle = this.t('infoBox.wait')
                        break
                    case 'success':
                        opts.title = this.t('infoBox.success')
                        opts.subtitle = this.t('infoBox.toBeContinue') + '>>'
                        break
                    case 'error':
                        opts.title = this.t('infoBox.danger')
                        opts.subtitle = this.t('infoBox.close')
                        break
                    case 'warning':
                        opts.title = this.t('infoBox.warning')
                        break
                }
